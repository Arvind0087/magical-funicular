...
const { getBase64ExtensionTypeTwo, getBase64ContentType } = require("./service");
const { config } = require("../config/db.config");

const uploadWordFile = async (file, s3directorypath) => {
  try {
    ...
    console.log('s3response', s3response.key);
    return s3response;
  } catch (error) {
    return false;
  }
